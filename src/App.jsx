/*  App.jsx ─ DXF → BMP monocromo (1 bpp)
    Versión “inversion‑fix”: la paleta del BMP queda
    índice 0 = negro (0,0,0), índice 1 = blanco (255,255,255)
    y los bits del bitmap usan 0 = negro, 1 = blanco.
*/
...
/*───────────────────────────────────────────────────────────────────────────
  Construye un BMP 1 bpp genuino con cabecera, paleta y datos bit‑packed
  Paleta:
    índice 0 = negro  (0,0,0)
    índice 1 = blanco (255,255,255)
───────────────────────────────────────────────────────────────────────────*/
function encode1BitBmp(width, height, monoData) {
  ...
  const bfOffBits  = 14 + biSize + 2 * 4;          // fileHdr + infoHdr + paleta
  ...
  dv.setUint32(p,   2835, true);           p += 4; // ppmX ≈ 72 dpi
...
/* Devuelve puntos relevantes para calcular el bounding box de la entidad */
...
/* Dibuja una entidad DXF con stroke negro */
...
/* Cualquier pixel que no sea blanco puro → negro puro */
...
/*───────────────────────────────────────────────────────────────────────────
  Convierte el RGBA del canvas a un Uint8Array 1 bpp (bottom‑up, padded 4 bytes)
  Lógica invertida respecto a la versión previa:
    bit 1 (valor 1) → fondo blanco
    bit 0 (valor 0) → trazo negro
───────────────────────────────────────────────────────────────────────────*/
function rgbaTo1Bit(width, height, rgbaData) {
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      /* trazo negro mantiene bit 0 (default) */
    }
  }
  return out;
}
...
function App() {
    ...
    new Promise((resolve, reject) => {
      ...
      reader.onload = async (e) => {
        try {
          ...
          /* ─ Bounding box y transformaciones ─ */
          ...
          ctx.scale(scale, -scale);          // eje Y invertido
          dxf.entities.forEach(ent => drawEntity(ctx, ent));
          ctx.restore();

          /* ─ Forzar negro puro y exportar ─ */
  ...
  return (
    <div className="App">
      <h1>DXF → BMP Converter (1‑bit, inversion fixed)</h1>
      ...
      {errors.length > 0 && (
        <div className="errors">
          ...
          <ul>
            {errors.map((e, i) =>
              <li key={i}><strong>{e.fileName}:</strong> {e.message}</li>)}
          </ul>
        </div>
      )}
